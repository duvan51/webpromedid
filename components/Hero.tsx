...
const Hero: React.FC<HeroProps> = ({ onServiceSelect, onTreatmentsClick }) => {
  return (
    <div className="relative min-h-[90vh] flex items-center overflow-hidden bg-emerald-50 pt-20">
      {/* Background elements */}
      <div className="absolute top-0 right-0 w-1/3 h-full bg-emerald-100/50 -skew-x-12 transform translate-x-1/4"></div>
      ...
      <div className="container mx-auto px-4 md:px-6 grid lg:grid-cols-2 gap-12 items-center relative z-10">
        <div className="space-y-8 animate-fade-in">
          <div className="inline-flex items-center gap-2 bg-emerald-100 text-emerald-700 px-4 py-1.5 rounded-full text-sm font-semibold">
            ...
            Líderes en Salud Holística en Colombia
          </div>

          <h1 className="text-5xl md:text-7xl font-bold text-emerald-950 leading-tight">
            Bienvenidos al mundo de la <span className="text-emerald-600">renovación total</span> y el bienestar integral
          </h1>

          <p className="text-lg text-slate-600 max-w-lg leading-relaxed">
            Transformamos vidas a través de medicina alternativa humanizada y tecnología de vanguardia. Descubra la raíz de sus dolencias y recupere su vitalidad.
          </p>

          <div className="flex flex-col sm:flex-row gap-4">
            <button
              onClick={() => onServiceSelect('analizador-cuantico')}
              className="bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-4 rounded-xl text-lg font-bold transition-all shadow-xl shadow-emerald-600/30 flex items-center justify-center gap-2 group"
            >
              Agendar Analizador Cuántico
            ...
            <button
              ...
              className="bg-white hover:bg-slate-50 text-emerald-700 border-2 border-emerald-100 px-8 py-4 rounded-xl text-lg font-bold transition-all flex items-center justify-center gap-2"
            >
              Ver Portafolio
            </button>
          </div>
